...
import { useGetQuotesQuery, useToggleFakeMutation, useDeleteQuotesMutation } from '../state/quotesApi'
export default function Quotes() {
  ...
  const [deleteQuote,  {isLoading: deletingQuote}] = useDeleteQuotesMutation()
    ...
    <div id="quotes">
      <h3>Quotes
        {(deletingQuote && ' in deletion...')}
         {(togglingQuote && ' being toggles...')}
         {(gettingQuotes || refreshingQuotes) && ' loading...'}</h3>
      <div>
        {
          quotes?.filter(qt => {
